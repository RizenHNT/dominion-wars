...
$lines.Add('')
$lines.Add('This is a static inventory against the historical Java implementation, not a parity requirement or approval. C# and Contract 1.31 are the active runtime boundary.')
$lines.Add('')
$lines.Add('## Effects')
$lines.Add('')
...
$lines.Add('')
$lines.Add('Target inventory is syntax-level only. Contract 1.31 and the card schema remain authoritative; historical Java aliases do not authorize silently selecting or widening a target.')
$lines.Add('')
...
$lines.Add('')
$lines.Add('Known non-equivalence: Java retains historical ambush/chant/discard-hook and LeaderDef fields. Do not treat this inventory as permission to expand the C# model or restore obsolete behavior.')
$lines.Add('')
$lines.Add('## Follow-up queue')
$lines.Add('')
$lines.Add('- Treat C#/Java differences as migration inventory only; do not change current C# behavior merely to match Java.')
$lines.Add('- Contract or rule changes remain PL/HUMAN_REQUIRED even when a historical Java behavior exists.')
$lines.Add('- Unity runtime validation is independent of this report; use `scripts/run-unity-runtime-validation.ps1` for current evidence.')
