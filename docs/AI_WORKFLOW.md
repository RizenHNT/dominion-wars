...
1. Claude 担任长期 PL；Claude 不可用期间由 DeepSeek V4 Flash（PL）临时代管，负责产品规划、规则提案、UX 方案、验收标准和实施清单。涉及表现层时，以 `design/runtime-kit-v1.30/contracts/` 为设计基线。MiniMax 不在当前 PL/审核执行路径中。
...
- `code chat` 只面向普通聊天视图，不能作为“已送达 VS Code 智能体窗口 DeepSeek V4 Flash PL”的证据。`scripts/auto-relay/notify-vscode-pl.ps1` 当前强制拒绝执行，直到存在可选择并验证目标智能体窗口的正式接口。无人值守协作继续使用已审计的后台接力控制器。
