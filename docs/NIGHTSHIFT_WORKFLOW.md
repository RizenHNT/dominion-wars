...
Windows Task Scheduler invokes `invoke-scheduled-nightshift.ps1`, a minimal watchdog that then starts the runner with `-Scheduled`. At launch, the watchdog reads only the human-owned `docs/DAILY_GOAL.md` from the main worktree, rejects links and oversized input, and atomically snapshots it into the private state directory; production files and other main-worktree changes are never copied. The isolated runner consumes that private snapshot, so the night-shift worktree remains clean. The watchdog binds status to a fresh per-launch identifier, contains the runner process tree, and enforces a deadline shorter than the scheduler's final limit. Even a missing/broken goal or config produces `BOOTSTRAP_FAILED` state. A missing `Status: READY` goal is a successful no-op. A READY goal is accepted only during the configured early-morning window, while its original file timestamp is fresh, and when its `Date:` is today or yesterday. Start/finish/failure records are appended to the private-state `scheduler.log`.
