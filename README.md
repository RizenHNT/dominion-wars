...
- **纳维恩弃牌联动**：新增「对方弃牌时」触发（onOpponentDiscardEffects），编辑器已支持。
...
当前世界观 Canon 见 [`docs/WORLD_LORE.md`](docs/WORLD_LORE.md)。世界内主要来源势力改用正式历史名称：
...
卡牌机制是对历史人物、事件和传说的游戏化抽象，**不反向决定世界历史**。例如惩罚值、弃牌、破城、特殊胜利等均不应被当作世界内事件的字面物理规则。
...
- 4 个主要来源势力 91 张卡、4 套 61 张构筑卡组、35 项规则回归测试、AI 平衡模拟器

完整规则见 [docs/RULES.md](docs/RULES.md)，世界观基准见 [docs/WORLD_LORE.md](docs/WORLD_LORE.md)，架构与扩展指南见 [docs/DESIGN.md](docs/DESIGN.md)，平衡数据见 [docs/BALANCE.md](docs/BALANCE.md)。
...
data/cards/*.json     赫萨廷 / 纳维恩诸邑 / 依兰维索 / 克莱恩书院 / 中立
data/decks/*.json     四套预构筑卡组
docs/                 规则书 / 世界观 / 架构文档 / 平衡说明
