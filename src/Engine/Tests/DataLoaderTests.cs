...
namespace DominionWars.Engine.Tests
{
    public sealed class DataLoaderTests
    {
        ...
        [Test] public void MapsCardAndLeaderEffectSpecsWithoutLosingTargetFlags() { var cards = CardCatalog.LoadDirectory(Path.Combine(DataRoot, "cards")).Cards; var imp = cards["flame_imp"].OnPlayEffects.Single(); var strike = cards["flame_strike"].OnPlayEffects.Single(); var leader = cards["flame_leader"].LeaderEnterEffects.Single(); Assert.Multiple(() => { Assert.That(imp.Action, Is.EqualTo("DAMAGE")); Assert.That(imp.Target, Is.EqualTo("ENEMY_FACE")); Assert.That(imp.Amount, Is.EqualTo(1)); Assert.That(strike.KingSlayer, Is.True); Assert.That(leader.Target, Is.EqualTo("ALL_ENEMY_MINIONS")); Assert.That(cards["flame_leader"].LeaderPunishEffects, Has.Count.EqualTo(2)); }); }
